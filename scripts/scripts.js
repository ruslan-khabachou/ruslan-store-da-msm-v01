...
async function loadEager(doc) {
  const locale = getMetadata('locale') || getMetadata('language') || 'en';
  const language = locale.toLowerCase().split(/[-_]/)[0];
  doc.documentElement.lang = language;
  doc.documentElement.dir = ['ar', 'he', 'fa', 'ur'].includes(language) ? 'rtl' : 'ltr';
  doc.body.dataset.locale = locale.toLowerCase();
  decorateTemplateAndTheme();
